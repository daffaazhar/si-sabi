...
const EnterComponent = ({ active, currentPath, data }: { active: boolean; currentPath: string; data: SearchData }) => {
    ...
    active && (
      <i
        className={classnames('text-xl', {
          ...
          'text-primary': currentPath === data.url
        })}
      />
    )
  )
      ...
      <div
        ...
        className={classnames(
          'flex items-center justify-between gap-4 relative plb-2 pli-4 cursor-pointer rounded-md',
          {
            ...
            'bg-[var(--mui-palette-primary-lightOpacity)] text-primary': !active && currentPath === data.url,
            'bg-[var(--mui-palette-primary-mainOpacity)] text-primary': active && currentPath === data.url
          }
        )}
      >
        ...
        <EnterComponent active={active} currentPath={currentPath} data={data} />
